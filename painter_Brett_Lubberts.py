...
def evaluate(im: np.ndarray):
	...
	flattened = im.reshape(-1, im.shape[2])
	unique = np.unique(flattened, axis=0)
	unique_colors = unique.shape[0]

	Horizontal = np.sum(np.abs(im[1:, :, :] - im[:-1, :, :]))

	# make sure horizontal doesn't overwhelm the unique colors
	evaluation = (50000 * unique_colors) + Horizontal
	return evaluation
...
def main():
	...
	parser.add_argument('-g', '--generations',
						default=100, help="The number of generations to run", type=int)
	parser.add_argument('-p',
						'--pools', default=10, help="The size of the pool", type=int)
	parser.add_argument('-m', '--mutation', default=.2,
